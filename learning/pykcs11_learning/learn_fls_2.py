...
def learn_fls_2_aes_ecb(lib: PyKCS11Lib, so: str, slot: int, pin: str, input_file: Path, output_path: Path):
    ...
    def _learn_fls_2_aes_ecb(session: Session) -> MealyMachine:
    ...
    automaton = run(lib, so, slot, pin, _learn_fls_2_aes_ecb)
...
def learn_fls_2_aes_gcm(lib: PyKCS11Lib, so: str, slot: int, pin: str, input_file: Path, output_path: Path):
    configuration = Configuration.load_from_file(input_file)
...
def learn_opencryptoki(input_path: Path):
    ...
    for input_file in input_path.glob("*.toml"):
        learn_fls_2_aes_ecb(lib, so, slot, pin, input_file, output_path)
        learn_fls_2_aes_gcm(lib, so, slot, pin, input_file, output_path)
...
def main():
    parser = argparse.ArgumentParser(description="Learn FLS 2 attack")
    ...
    input_path = Path("examples", "fls_2")
