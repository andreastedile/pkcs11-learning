...
def learn_wrap_and_decrypt_aes_ecb(lib: PyKCS11Lib, so: str, slot: int, pin: str, input_file: Path, output_path: Path):
    ...
    def _learn_wrap_and_decrypt_aes_ecb(session: Session) -> MealyMachine:
    ...
    automaton = run(lib, so, slot, pin, _learn_wrap_and_decrypt_aes_ecb)
...
def learn_wrap_and_decrypt_aes_gcm(lib: PyKCS11Lib, so: str, slot: int, pin: str, input_file: Path, output_path: Path):
    configuration = Configuration.load_from_file(input_file)
...
def learn_opencryptoki(input_path: Path):
    ...
    for input_file in input_path.glob("*.toml"):
        learn_wrap_and_decrypt_aes_ecb(lib, so, slot, pin, input_file, output_path)
        learn_wrap_and_decrypt_aes_gcm(lib, so, slot, pin, input_file, output_path)
...
def main():
    parser = argparse.ArgumentParser(description="Learn wrap and decrypt attack")
    ...
    input_path = Path("examples", "wrap_and_decrypt")
